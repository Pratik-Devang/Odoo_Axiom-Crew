...
const DISPLAY_COLORS = ['Blue', 'Red', 'Green', 'Orange', 'Purple', 'Yellow', 'Gray'];
...
export function defaults(collection: string, s: Workspace, employeeId?: string): Row {
  ...
  const activeEmployees = s.employees.filter((item) => item.status !== 'Archived');
  const defaultEmployee = activeEmployees[0];
  const records: Record<string, any> = {
    employees: { name: '', email: '', phone: '', department: defaultEmployee?.department || 'Engineering', position: '', type: 'Full-time', status: 'Active', manager: defaultEmployee?.name || '', location: defaultEmployee?.location || 'Mumbai', scheduleId: schedule, bank: '' },
    ...
    requests: { employeeId: employee, typeId: s.leaveTypes[0]?.id || 'paid', start: '2026-09-14', end: '2026-09-14', duration: 1, reason: '', status: 'Pending' },
    allocations: { employeeId: employee, typeId: s.leaveTypes[0]?.id || 'paid', amount: 20, start: '2027-01-01', end: '2027-12-31', status: 'Pending' },
    leaveTypes: { name: '', unit: 'Days', requiresAllocation: true, approvalWorkflow: 'HR Approval', payrollImpact: 'Paid', payrollWorkEntry: '', displayColor: 'Blue', active: true },
...
export default function RecordForm({ collection, initial, s, busy, onSave, onDelete, onCancel }: Props) {
  ...
  return <form onSubmit={(event) => { event.preventDefault(); void onSave(r); }}>
    <div className="form-grid">
      ...
      {collection === 'leaveTypes' && <>{input('name', 'Type name')}{pick('unit', 'Unit', ['Days', 'Hours'])}{pick('approvalWorkflow', 'Approval workflow', ['No Approval', 'Manager Approval', 'HR Approval'])}{pick('payrollImpact', 'Payroll treatment', ['Paid', 'Unpaid', 'No Payroll Impact'])}{input('payrollWorkEntry', 'Payroll / work entry type', 'text', false)}{pick('displayColor', 'Display color', DISPLAY_COLORS)}<label className="check-label"><Checkbox checked={!!r.requiresAllocation} onCheckedChange={(value) => set('requiresAllocation', !!value)} />Requires an approved allocation</label><label className="check-label"><Checkbox checked={r.active !== false} onCheckedChange={(value) => set('active', !!value)} />Active</label></>}
