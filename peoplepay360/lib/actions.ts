...
export function mutate(source:Workspace,action:string,p:Record<string,any>,actor='System'):Workspace{
 ...
 if(action==='save'){
  ...
  if(collection==='requests'||collection==='allocations'){
   ...
   else{const t=s.leaveTypes.find(t=>t.id===r.typeId)!;requireThat(t.unit!=='Hours'||r.start===r.end,'Hourly leave must be on a single date.');if(t.unit==='Days')r.duration=workingDaysBetween(s,r.employeeId,r.start,r.end);else{requireThat(!!scheduleRowForDate(employeeSchedule(s,r.employeeId,r.start),r.start),'Hourly leave must fall on a scheduled working day.');r.duration=+r.duration;}requireThat(numeric(r.duration)&&r.duration>0,'The request must include scheduled working time.');requireThat(text(r.reason),'A reason is required.');requireThat(!s.requests.some(a=>a.id!==r.id&&a.employeeId===r.employeeId&&a.status!=='Refused'&&a.start<=r.end&&r.start<=a.end),'This employee already has leave requested on these dates.');if(t.approvalWorkflow==='No Approval'){if(t.requiresAllocation){const a=s.allocations.find(a=>a.employeeId===r.employeeId&&a.typeId===r.typeId&&a.status==='Approved'&&a.start<=r.start&&a.end>=r.end);requireThat(a&&allocationBalance(s,a)>=r.duration,'Insufficient approved leave allocation for these dates.');r.allocationId=a!.id;}r.status='Approved';r.approver='Automatic policy';}}
  }
  if(collection==='leaveTypes'){requireThat(['Days','Hours'].includes(r.unit),'Invalid leave unit.');requireThat(['No Approval','Manager Approval','HR Approval'].includes(r.approvalWorkflow),'Select an approval workflow.');requireThat(['Paid','Unpaid','No Payroll Impact'].includes(r.payrollImpact),'Select a payroll treatment.');r.requiresAllocation=!!r.requiresAllocation;r.payrollWorkEntry=r.payrollWorkEntry||'';r.displayColor=r.displayColor||'Blue';r.active=r.active!==false;requireThat(!old||(!s.requests.some(q=>q.typeId===old.id)&&!s.allocations.some(a=>a.typeId===old.id))||(old.unit===r.unit&&old.requiresAllocation===r.requiresAllocation),'Cannot change units or allocation policy after this type has been used.');}
  if(collection==='rules'){
  ...
  if(collection==='schedules'){const ts=(t:any)=>typeof t==='string'&&/^([01]\d|2[0-3]):[0-5]\d$/.test(t);requireThat(['Fixed','Flexible','Shift'].includes(r.type),'Select a schedule type.');requireThat(Array.isArray(r.workRows),'Add weekly schedule rows.');const rows=scheduleRows(r);requireThat(rows.some(row=>row.working),'Select at least one working day.');rows.filter(row=>row.working).forEach(row=>{requireThat(ts(row.start)&&ts(row.end)&&row.end>row.start,`Enter valid hours for ${row.day}.`);requireThat(numeric(row.breakHours),'Enter valid break hours.');row.breakHours=+row.breakHours;const span=(+row.end.slice(0,2)*60 + +row.end.slice(3)-(+row.start.slice(0,2)*60 + +row.start.slice(3)))/60;requireThat(row.breakHours<span,`Break must be shorter than the ${row.day} shift.`);});r.workRows=rows;r.days=rows.filter(row=>row.working).map(row=>row.day);const first=rows.find(row=>row.working)!;r.start=first.start;r.end=first.end;r.breakHours=first.breakHours;r.weeklyHours=scheduleWeeklyHours(r);r.company=typeof r.company==='string'&&r.company.trim()?r.company.trim():'My Company';r.timezone=typeof r.timezone==='string'&&r.timezone.trim()?r.timezone.trim():'Company timezone';r.status=['Active','Inactive'].includes(r.status)?r.status:'Active';}
  if(old)s[collection]=s[collection].map(x=>x.id===r.id?r:x);else s[collection].push(r);
  if(['rules','structures','contracts','attendance','requests','leaveTypes','schedules'].includes(collection))s.payruns.forEach(run=>{if(run.status==='Computed'){run.status='Draft';run.slips=[];}});
 }
  ...
  if(collection==='employees'){record!.status='Archived';}
   ...
   if(['contracts','attendance','rules','structures','requests','leaveTypes','schedules'].includes(collection))s.payruns.forEach(run=>{if(run.status==='Computed'){run.status='Draft';run.slips=[];}});
  }
 }
