...
export function attendanceInPeriod(attendance: Row[], employeeIds: Set<string>, period: string): Row[] {
...
export function attendanceForEmployee(attendance: Row[], employeeId: string, period: string): Row[] {
  const start = period + '-01';
  const end = monthEnd(period);
  return attendance.filter((a) => a.employeeId === employeeId && a.date >= start && a.date <= end);
}
...
export function getEmployeeAttendanceRate(
  ...
  return Math.round((rows.filter((a) => Boolean(a.checkIn)).length / rows.length) * 100);
}

/* ─── Payroll estimates ─── */

export function estimatePeriodNet(s: Workspace, employeeIds: Iterable<string>, period: string): number {
  let total = 0;
...
export function filterEmployees(s: Workspace, filters: DashboardFilters): Row[] {
...
export function getWorkforceHealth(
  ...
  const healthRate = attendance.length ? Math.round((complete.length / attendance.length) * 100) : null;
...
export function getDepartmentShare(
  ...
  const rows = departments
    ...
    .map((d) => {
      ...
      const deptEstimated = estimatePeriodNet(s, deptIds, period);
      const amount = deptSlipsAmount > 0 ? deptSlipsAmount : deptEstimated;
...
export function getNetSalaryTrend(
  ...
  return Array.from({ length: months }, (_, i) => {
    ...
    const estimatedForPeriod = estimatePeriodNet(s, employeeIds, p);

    return {
      period: p,
      label: monthShort(p),
      value: realNet > 0 ? realNet : estimatedForPeriod,
...
export function getTopNetPay(
  ...
  const earners = activeEmployees
    .map((e) => {
      ...
      const { net, hasSlip } = employeeNetForPeriod(s, e.id, period, slip);
...
export function getEmployeeRosterRows(
  ...
  return filterEmployeesWithSearch(s, filters, query).map((e) => ({
    ...
    type: e.type,
    status: e.status,
    attendanceRate: getEmployeeAttendanceRate(s.attendance, e.id, filters.period),
  }));
}

/** Full dashboard snapshot — pure function, no React. */
export function buildDashboardSnapshot(s: Workspace, filters: DashboardFilters): DashboardSnapshot {
  ...
  const estimatedNet = estimatePeriodNet(s, filteredEmployeeIds, filters.period);
  const totalNet = hasActualPayroll ? slips.reduce((n, p) => n + p.net, 0) : estimatedNet;
  const totalGross = hasActualPayroll
    ? slips.reduce((n, p) => n + (p.gross || 0), 0)
    : activeEmployees.reduce((n, e) => n + employeeNetForPeriod(s, e.id, filters.period).gross, 0);
  ...
  const workforceHealth = getWorkforceHealth(s, filteredEmployeeIds, filters.period);
  ...
  const { earners: topEarners, maxNet: maxEarnerNet } = getTopNetPay(s, activeEmployees, slips, filters.period);
